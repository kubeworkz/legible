...
Examples:
  legible summary --question "How many orders?" --sql "SELECT count(*) FROM orders"
  legible summary -q "Top customers by revenue" -s "SELECT customer_id, sum(price) FROM order_items GROUP BY 1 ORDER BY 2 DESC LIMIT 5"`,
	RunE: runSummary,
}
