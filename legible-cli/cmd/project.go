...
operate on that project. Use "legible project list" to see available projects.

Examples:
  legible project use 1
  legible project use 42`,
...
Examples:
  legible project update --name "My Project"
  legible project update 2 --language en --timezone "Asia/Tokyo"`,
	Args: cobra.MaximumNArgs(1),
...
func runProjectCurrent(cmd *cobra.Command, args []string) error {
	...
	if cfg.ProjectID == "" {
		...
		fmt.Println("No project selected. Use: legible project use <id>")
		return nil
	}
...
func runProjectInfo(cmd *cobra.Command, args []string) error {
	...
	if len(args) == 1 {
		...
		return fmt.Errorf("no project specified — pass an ID or set one with: legible project use <id>")
	}
