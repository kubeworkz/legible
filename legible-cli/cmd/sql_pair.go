...
Examples:
  legible sql-pair create --question "Total revenue" --sql 'SELECT sum(revenue) FROM "orders"'
  legible sql-pair create -q "Count customers" -s 'SELECT count(*) FROM "olist_customers_dataset"'`,
...
Examples:
  legible sql-pair update 1 --question "Updated question"
  legible sql-pair update 1 --sql 'SELECT new_query FROM "table"'`,
	Args: cobra.ExactArgs(1),
