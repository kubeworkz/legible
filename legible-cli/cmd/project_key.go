...
Examples:
  ...
  legible project-key list --project-id 2`,
...
Examples:
  legible project-key create "CI/CD Key"
  legible project-key create "Read-Only" --project-id 2`,
...
func resolveProjectKeyPID(cfg *config.Config) (int, error) {
	...
	if cfg.ProjectID == "" {
		return 0, fmt.Errorf("no project ID specified; use --project-id or set via 'legible project use <id>'")
	}
