...
SQL_FILE="${ROOT_DIR}/ism-data/postgres/sql/202_build_curated_fact_views.sql"

if [[ ! -f "${SQL_FILE}" ]]; then
  echo "Curated fact-view SQL not found: ${SQL_FILE}" >&2
  exit 1
fi
...
echo "Curated fact views build complete."
