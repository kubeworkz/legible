...
SQL_FILE="${ROOT_DIR}/ism-data/postgres/sql/007_build_bridge_candidates_latest_view.sql"

if [[ ! -f "${SQL_FILE}" ]]; then
  echo "Latest-candidate view SQL not found: ${SQL_FILE}" >&2
  exit 1
fi
...
docker compose -f "${COMPOSE_FILE}" exec -T "${PG_SERVICE}" \
  psql -U "${PG_USER}" -d "${PG_DB}" -c "
SELECT source_name, security_code, occurrence_count, candidate_security_no, candidate_reason, candidate_confidence, review_status
FROM ops.bridge_remediation_candidates_latest
ORDER BY occurrence_count DESC, source_name, security_code
LIMIT 25;
"

echo "Latest remediation candidate view build complete."
