...
SQL_FILE="${ROOT_DIR}/ism-data/postgres/sql/005_build_bridge_remediation_candidates.sql"

if [[ ! -f "${SQL_FILE}" ]]; then
  echo "Bridge remediation SQL not found: ${SQL_FILE}" >&2
  exit 1
fi
...
SELECT source_name, security_code, occurrence_count, candidate_security_no, candidate_reason, candidate_confidence
FROM ops.bridge_remediation_candidates
WHERE generated_at = (SELECT max(generated_at) FROM ops.bridge_remediation_candidates)
ORDER BY occurrence_count DESC, source_name, security_code
LIMIT 25;
"

echo "Bridge remediation candidate build complete."
