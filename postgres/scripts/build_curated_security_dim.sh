...
SQL_FILE="${ROOT_DIR}/ism-data/postgres/sql/201_build_curated_security_dim.sql"

if [[ ! -f "${SQL_FILE}" ]]; then
  echo "Curated security dim SQL not found: ${SQL_FILE}" >&2
  exit 1
fi
...
SELECT security_origin, quality_tier, count(*) AS rows
FROM curated.dim_security
GROUP BY security_origin, quality_tier
ORDER BY security_origin, quality_tier;
"

echo "Curated security dimension build complete."
